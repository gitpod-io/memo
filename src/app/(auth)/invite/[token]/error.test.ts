...
describe("invite route error boundary", () => {
  it("error.tsx exists for the invite route segment", () => {
  ...
  it("shows invite-specific messaging", () => {
    const source = readFileSync(resolve(DIR, "error.tsx"), "utf-8");
    expect(source).toContain("Could not load invite");
  });
